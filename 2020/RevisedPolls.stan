data{
    ...
    vector [S] mu_b_prior;
    matrix [S, S] sigma_mu_b_end;
    matrix [S, S] sigma_walk_b_forecast;
}

transformed data{
    matrix [S, S] chol_sigma_walk_b_forecast; 
    matrix [S, S] chol_sigma_mu_b_end;
    vector [S] zero_vec;
    int last_tuesday;
    ...
    for (state in 1:(S)) zero_vec[state] = 0;
...
transformed parameters{
    ...
    matrix[W,S] mu_b;
    matrix[T_unique , S] matrix_inv_logit_mu_ab;
    real mu_a_t;
    ...
    mu_b[W, 1:S] = to_row_vector(mu_b_prior[1:S] + chol_sigma_mu_b_end * to_vector(mu_b_end[1:S]));
    if (last_poll_W < W){
        for (wk in 1:(W - last_poll_W))
            mu_b[W - wk, 1:S] = mu_b[W - wk + 1, 1:S] + 
                                to_row_vector(chol_sigma_walk_b_forecast * to_vector(delta_b[W - wk, 1:S]));
    }
    for (wk in (W - last_poll_W + 1):(W-1))
        mu_b[W - wk, 1:S] = mu_b[W - wk + 1, 1:S] + sigma_walk_b_past * sqrt(7) * delta_b[W - wk, 1:S];
    for (wk in 1:W) 
        mu_b[wk, 1] = 0;
    for(i in 1:N){
        logit_clinton_hat[i] = mu_a[t[i]] + mu_b[w[i], s[i]];
    }
...
model{
    ...
    n_dem ~ binomial_logit(n_respondents, logit_clinton_hat);
}
